...
class NetworkManager {
    ...
    static func initNewsForPrefetching(_ timeInterval1970: String, forController controller: NewsViewController, next_from: String) {
        var urlConstructor = URLComponents()
        urlConstructor.path = METHOD_PATH + API_NEWSFEED_GET
        urlConstructor.scheme = HTTPS_SCHEME
        urlConstructor.host = VK_HOST
        ...
        queue.addOperation {
            let task = session.dataTask(with: urlConstructor.url!) { data, response, error in
                do {
                    ...
                    let newsItemsResponse = try JSONDecoder().decode(NewsItemResponseContainer.self, from: data!)
                    print("LOADED NEWS")
                    controller.nextForm = newsItemsResponse.response.next_form ?? ""
                    controller.endPrefetching(news: newsItemsResponse.response.items)
                } catch (let error) {
                    print(error)
                }
            }
